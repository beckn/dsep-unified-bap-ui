   ...
   <ScrollView >
     ...
     {loader && <Loader />}
        ...
        <View style={styles.card} >
                 ...
                 <Text>{moment(data?.scholarshipProviders[0]?.scholarships[0]?.applicationStartDate).format('DD-MM-YYYY')}</Text>
        </View>
        <View style={styles.line}></View>
        <View style={styles.card} >
                 <Text style={styles.heading}>{'Application End Date'}</Text>
                 <Text>{moment(data?.scholarshipProviders[0]?.scholarships[0]?.applicationEndDate).format('DD-MM-YYYY')}</Text>
        </View>
        <Spacer size={20}/>
    </SafeAreaView>
    ...
    <Button onPress={onClickApply} 
      disabled={true}
      text={data?.scholarshipProviders[0].scholarships? data?.scholarshipProviders[0]?.scholarships[0]?.scholarshipDetails.scholarshipStatus.code: "no status"} type="dark" 
      />
       <Spacer size={10}/>
      </View> 
   </ScrollView>
